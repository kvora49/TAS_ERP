-- Migration: 20260805000200_fix_payment_rpc_advance_flag.sql
-- Description: Fix record_payment RPC so is_advance is true ONLY when no bills are allocated (v_allocated_total = 0).
...
BEGIN
  ...
  -- is_advance is TRUE only if NO bills were allocated (v_allocated_total = 0)
...
-- Data Cleanup: Fix existing payments that have allocations but were wrongly marked as is_advance = true
UPDATE payments
SET is_advance = false
