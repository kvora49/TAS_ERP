...
export default function GeneralSettingsPage() {
  ...
  const { setMotionProfile } = useExperienceController();

  const { data, isLoading, error, refetch } = useGeneralSettings();
  ...
  const [enableKachaBilling, setEnableKachaBilling] = useState(true);
  const [lowStockAlerts, setLowStockAlerts] = useState(true);
  const [allowNegativeStock, setAllowNegativeStock] = useState(false);
  const [motionProfileState, setMotionProfileState] = useState<"ultraFast" | "balanced" | "premium">("balanced");
  ...
  useEffect(() => {
    ...
    if (data?.settings) {
      setEnableKachaBilling(data.settings.enable_kacha_billing ?? true);
    ...
    mutationFn: async () => {
      ...
      const res = await fetch("/api/settings/general", {
        ...
        body: JSON.stringify({
          ...
          motion_profile: motionProfileState,
          client_updated_at: data?.business?.updated_at,
        }),
      });

      const resData = await res.json();
      if (!res.ok) {
        throw new Error(resData.error || "Failed to update general settings");
      }
      return resData;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings", "general"] });
      setMotionProfile(motionProfileState);
  ...
  const activePrefs = [
    ...
    { name: "Enable Kaacha Billing", active: enableKachaBilling },
      ...
      <div className="flex flex-col gap-6 pb-20 md:pb-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col gap-5 text-left">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full h-10 rounded-lg border border-[var(--input-border)] px-3 text-sm bg-[var(--input-bg)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--input-focus)] focus:border-transparent transition-colors"
                >
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-muted)] mt-1.5">System time zone</p>
              </div>
          ...
          >
            <div className="flex flex-col gap-0 text-left">
              ...
              <SettingsToggleRow
                ...
                label="Enable Kaacha (Estimate) Billing"
                subtitle="Allow creating Kaacha estimate bills alongside Tax Invoices"
                checked={enableKachaBilling}
                onCheckedChange={setEnableKachaBilling}
              />
              <SettingsToggleRow
                icon={Boxes}
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              <div>
                <h4 className="text-sm font-semibold text-[var(--badge-green-text, #15803D)] mb-3 text-left">
                ...
                <div className="flex flex-col gap-1.5">
                  {activePrefs.map((pref, i) => (
                    <div key={i} className="flex items-center gap-2 py-1.5 text-left">
                      <CheckCircle2 className="size-4 text-[var(--badge-green-text, #15803D)] shrink-0" />
                      <span className="text-sm text-[var(--text-body)]">{pref.name}</span>
                    </div>
                  ))}
                  {activePrefs.length === 0 && (
                    <span className="text-xs text-[var(--text-faint)] italic text-left block">
