-- Migration: Harden payment and advance settlement RPCs with strict tenant business_id isolation
-- Prevents cross-tenant bill status corruption when allocations are passed to RPCs

-- 1. Settle Advance Multi RPC (Strict Multi-Tenant Scoping)
...
BEGIN
  -- 1. Fetch advance record details (Strictly scoped to business_id)
  ...
  IF v_payment_id IS NULL THEN
    RAISE EXCEPTION 'Advance record not found or does not belong to active company';
  END IF;
  ...
  SET settled_amount = COALESCE(settled_amount, 0) + v_total_to_settle,
      ...
      updated_at = NOW()
  WHERE (id = p_advance_id OR payment_id = v_payment_id) AND business_id = p_business_id;
  ...
  SET unallocated_amount = GREATEST(0, COALESCE(unallocated_amount, 0) - v_total_to_settle),
      updated_at = NOW()
  WHERE id = v_payment_id AND business_id = p_business_id;

  -- 3. Loop over allocations, insert payment_allocations, and update respective bills (strictly scoped to p_business_id)
  FOR v_alloc IN 
    SELECT * FROM jsonb_to_recordset(p_allocations) AS x(
      "billId" UUID, "allocatedAmount" NUMERIC(15,2), "billType" TEXT,
      bill_id UUID, allocated_amount NUMERIC(15,2), bill_type TEXT
  ...
  LOOP
    ...
    IF v_alloc_amount > 0 AND v_target_bill_id IS NOT NULL THEN
      ...
      IF v_target_bill_type = 'sale_bill' THEN
        ...
        FROM sale_bills WHERE id = v_target_bill_id AND business_id = p_business_id;
        ...
        IF FOUND THEN
          ...
          WHERE id = v_target_bill_id AND business_id = p_business_id;
        END IF;

      ELSIF v_target_bill_type = 'raw_material_purchase' THEN
        SELECT COALESCE(paid_amount, 0), COALESCE(grand_total, 0) INTO v_current_paid, v_grand_total
        FROM raw_material_purchases WHERE id = v_target_bill_id AND business_id = p_business_id;
      ...
      ELSIF v_target_bill_type = 'purchase_bill' THEN
        SELECT COALESCE(paid_amount, 0), COALESCE(grand_total, 0) INTO v_current_paid, v_grand_total
        FROM purchase_bills WHERE id = v_target_bill_id AND business_id = p_business_id;
...
-- 2. Record Payment RPC (Strict Multi-Tenant Scoping)
...
BEGIN
  ...
  -- 5. Loop over allocations, insert payment_allocations, and update respective bills (strictly scoped to p_business_id)
  FOR v_alloc IN 
    SELECT * FROM jsonb_to_recordset(p_allocations) AS x(
      "billId" UUID, "allocatedAmount" NUMERIC(15,2), "billType" TEXT,
      bill_id UUID, allocated_amount NUMERIC(15,2), bill_type TEXT
  ...
  IF v_unallocated > 0 THEN
    IF EXISTS (SELECT 1 FROM advance_payments WHERE payment_id = v_payment_id AND business_id = p_business_id) THEN
      UPDATE advance_payments
      SET remaining_amount = v_unallocated, is_settled = false, updated_at = NOW()
      WHERE payment_id = v_payment_id AND business_id = p_business_id;
    ELSE
