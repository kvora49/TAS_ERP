...
CREATE POLICY "tenant_isolation" ON godown_transfers
  FOR ALL USING (business_id = (SELECT business_id FROM users WHERE id = auth.uid()));

-- Transfer items
CREATE TABLE godown_transfer_items (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id UUID NOT NULL REFERENCES businesses(id) ON DELETE CASCADE,
  transfer_id UUID NOT NULL REFERENCES godown_transfers(id) ON DELETE CASCADE,
  design_id UUID NOT NULL REFERENCES designs(id),
  colour_id UUID NOT NULL REFERENCES design_colours(id),
  size TEXT NOT NULL,
  from_godown_id UUID NOT NULL REFERENCES godowns(id),
...
CREATE POLICY "tenant_isolation" ON godown_transfer_items
  FOR ALL USING (business_id = (SELECT business_id FROM users WHERE id = auth.uid()));
...
CREATE TABLE challans (
  ...
  total_quantity INTEGER DEFAULT 0,
  total_value NUMERIC(15,2) DEFAULT 0,
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending','dispatched','in_transit','received','completed','cancelled')),
  created_by UUID REFERENCES users(id),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  deleted_at TIMESTAMPTZ,
...
CREATE POLICY "tenant_isolation" ON challans
  FOR ALL USING (business_id = (SELECT business_id FROM users WHERE id = auth.uid()));
CREATE TRIGGER set_updated_at BEFORE UPDATE ON challans
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TABLE barcode_scan_history (
  ...
  scan_result TEXT DEFAULT 'found' CHECK (scan_result IN ('found','not_found')),
  action_taken TEXT, -- 'add_to_bill', 'create_transfer', 'view_stock'
...
|---|---|---|
| GET | `/api/finished-stock` | Stock overview — all designs with totals |
| GET | `/api/finished-stock/stats` | 6 KPI stat cards + chart data |
| GET | `/api/finished-stock/chart/godown` | Stock by godown for donut chart |
| GET | `/api/finished-stock/chart/size` | Stock by size for bar chart |
| GET | `/api/finished-stock/designs` | All designs with stock quantities |
| GET | `/api/finished-stock/designs/[id]` | Design detail — colour/size matrix per godown |
| POST | `/api/finished-stock` | Create stock entry (from lot completion or manual) |
| GET | `/api/finished-stock/adjustments` | List with filters + stats |
| POST | `/api/finished-stock/adjustments` | Create adjustment + update finished_stock |
| GET | `/api/finished-stock/adjustments/[id]` | Adjustment detail |
| GET | `/api/finished-stock/transfers` | List with filters + stats |
| POST | `/api/finished-stock/transfers` | Create transfer + update stock both godowns |
| GET | `/api/finished-stock/transfers/[id]` | Transfer detail |
| GET | `/api/finished-stock/challans` | List with filters + stats |
| POST | `/api/finished-stock/challans` | Create challan |
| GET | `/api/finished-stock/challans/[id]` | Challan detail |
| PATCH | `/api/finished-stock/challans/[id]/return` | Return items — restores stock |
| PATCH | `/api/finished-stock/challans/[id]/convert` | Convert to bill (stub → Coming Soon) |
| PATCH | `/api/finished-stock/challans/[id]/partial` | Mark as partial |
| GET | `/api/finished-stock/barcode/scan` | Lookup by barcode code |
| POST | `/api/finished-stock/barcode/scan` | Record scan + create history |
| GET | `/api/finished-stock/barcode/scan-history` | Recent + all scan history |
| GET | `/api/finished-stock/check-stock` | Check availability for design+colour+size+godown |
...
| Day | Focus | Tasks |
|---|---|---|
| **Day 1** | Sidebar + DB + Overview | Add Finished Stock expandable to sidebar with all 6 sub-items + Scan (PWA). Run all Phase 5 SQL. Build Finished Stock Overview: filter bar, 6 stat cards, Stock by Design table with colour dots, right panel (donut chart + bar chart + quick actions). Wire Quick Stats to sidebar bottom. |
| **Day 2** | Stock Detail (per design) | Design header card with thumbnail + 4 KPI tiles, Godown/Brand filter row, Stock by Colour/Size matrix table (hierarchical colour groups + size sub-rows + total rows + godown columns). Export button. |
| **Day 3** | Adjustments List + Add Adjustment | Adjustments List: filter bar, 4 stat cards, table with colour-coded Qty Change / Value Impact. Add Adjustment: Adjustment Details section (type with icon, date, godown, reason, remarks), Stock Selection (design+colour+size+Check Stock → availability row → qty ± control → value impact). Right: Impact Summary panel + photo attachment with preview. |
| **Day 4** | Transfers List + Add Transfer | Transfers List: filter bar, 4 stat cards, table with status badges. Add Transfer: Transfer Details (from/to godown validation), Stock Selection (Check Stock → availability + reserved + free qty), qty ± control, Add Another Item, Items to Transfer table with footer totals. Right: Transfer Summary + info note. |
| **Day 5** | Challans List + Create Challan | Challans List: filter bar, 4 stat cards, table (Inward/Outward badges + all status badges), eye+dropdown action. Info banner. Create Challan: Challan Details (date, type, from godown, to party, ref, remarks), Add Items (same Check Stock pattern), Items Added table. Right: Challan Summary + Dispatch Info (transporter, LR/AWB, dispatched by, e-way bill). |
| **Day 6** | Challan Detail | Full challan detail: header card grid, Resolution Options 4-button row (Section 2.3), Items table, Item-wise Stock Summary 4 mini cards, Challan Timeline (4-step), right panel (Challan Summary + Stock Impact + Documents + Notes), blue info banner. |
| **Day 7** | Barcode/QR Scanning | Scan area with dashed border + manual input + tip banner, Scan Success banner, Recent Scans/Scan History tabs, scan history table. Right: Item Details panel (post-scan) + Actions list (5 items) + Quick Scan Tips. Footer: Clear + Export buttons. Wire html5-qrcode library. |
| **Day 8** | Scan (PWA) | Mobile-first scan page. Camera feed with scan frame (corner brackets), Auto Detect badge, zoom pills, success overlay. Recent Scans list. Right: Item Information card + Quick Actions tabs (Add to Bill/Transfer/View Stock) + Add to Bill form (party select, qty ± control, rate, amount, Add to Bill/Create New Bill buttons). Scan Tips. PWA bottom nav (5 items). |
| **Day 9** | Lot Complete → Finished Stock + QA | Wire Phase 4 "Mark Lot Complete" to create finished_stock entries. Test full flow: create lot → stage entries → mark complete → stock appears in Overview. Test Check Stock button updates across all 3 forms. Test barcode generation (qrcode lib) + scan lookup. |
| **Day 10** | Polish + Deploy | Test all CRUD. Verify challan resolution options (Return→stock restore, Convert to Bill→Coming Soon toast, Partial/Transfer work). Test colour dots render correctly everywhere. Mobile responsive check on all pages. PWA bottom nav works on mobile. Deploy. |
...
### Sidebar
- [ ] Finished Stock expandable with 6 sub-items
- [ ] Scan (PWA) top-level item at bottom of sidebar
- [ ] Quick Stats panel shows below user card
- [ ] All items navigate to correct routes

### Finished Stock Overview
- [ ] 6 stat cards with correct icons and colors
- [ ] Filter bar (6 filters) works and applies correctly
- [ ] Stock by Design table: colour dots with +N overflow render
- [ ] Eye icon navigates to Stock Detail
- [ ] Stock by Godown donut chart renders with legend
- [ ] Stock by Size bar chart renders with labels above bars
- [ ] Quick Actions 4 buttons navigate to correct pages

### Stock Detail
- [ ] Design thumbnail shows from R2 or placeholder
- [ ] 4 KPI tiles compute correctly
- [ ] Matrix table: colour groups + size sub-rows + colour total rows
- [ ] Godown columns are dynamic (shows columns for godowns that have stock)
- [ ] Total rows show correct sums per colour
- [ ] Export button downloads CSV

### Adjustments
- [ ] Adjustment Type dropdown shows colored icon per type
- [ ] Check Stock shows availability for selected design+colour+size+godown
- [ ] Qty ± control (−/input/+) works with validation
- [ ] Negative adjustment: "Quantity will be reduced" note shows
- [ ] Impact Summary updates live as qty changes
- [ ] Photo attachment preview shows uploaded image with × remove
- [ ] Save Adjustment updates finished_stock.size_quantities
- [ ] Adjustments list: Qty Change negative=red, positive=green

### Transfers
- [ ] From/To Godown validation: cannot be same godown
- [ ] Check Stock shows Available / Reserved / Free qty
- [ ] "Max available: X Pcs" shows below qty control
- [ ] Add Another Item adds to Items to Transfer table
- [ ] Transfer Summary updates live
- [ ] Save Transfer: deducts from from_godown, adds to to_godown in finished_stock

### Challans
- [ ] Outward badge=purple, Inward badge=green
- [ ] All 5 status badges correct colors
- [ ] Eye + dropdown action column renders
- [ ] Inward/Outward info banner at bottom
- [ ] Create Challan: Check Stock works same as Transfer/Adjustment
- [ ] Dispatch Info panel: all 4 fields optional
- [ ] Challan Detail: Resolution Options 4 buttons render with correct colors
- [ ] Return Items → restores stock + marks challan resolved
- [ ] Convert to Bill → Coming Soon toast (Phase 6)
- [ ] Mark as Partial → marks status partial
- [ ] Create Transfer → navigates to Add Transfer pre-filled
- [ ] Item-wise Stock Summary 4 cards compute correctly
- [ ] Challan Timeline 4 steps render with correct states

### Barcode/QR
- [ ] Dashed border scan zone with hover effect
- [ ] Manual barcode input works (lookup on Enter)
- [ ] Scan Success banner appears then can be dismissed
- [ ] Scan history table shows recent scans with In Stock/Not Found badges
- [ ] Item Details panel populates after scan
- [ ] 5 Action buttons navigate correctly
- [ ] Clear button resets the scan result

### Scan (PWA)
- [ ] Camera feed activates via html5-qrcode
- [ ] Corner bracket scan frame renders correctly
- [ ] Auto Detect ON badge with pulsing green dot
- [ ] Zoom controls switch camera zoom
- [ ] Success overlay shows 2s after scan then fades
- [ ] Recent Scans list updates with new scans
- [ ] Item Information card shows after scan
- [ ] Add to Bill tab: all fields, both buttons work
- [ ] PWA bottom nav: 5 items with Scan as center circle button
- [ ] Page works correctly on mobile (375px width)
- [ ] PWA installable (manifest + service worker)

---

*TAS ERP Phase 5 Implementation Plan | UI-Accurate | June 2026*
